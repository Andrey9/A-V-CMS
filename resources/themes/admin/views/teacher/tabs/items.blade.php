                        ...
                        <div class="tab-content">
                            @foreach (config('app.locales') as $key => $locale)
                                <div class="tab-pane fade in @if ($key == 0) active @endif" id="tab_item_locale_old_{!! $locale !!}_{!! $item->id !!}">
                                    @include('views.teacher.tabs.item_locale', ['id' => $item->id, 'key' => 'old'])
                                </div>
                            @endforeach
                        </div>
                    </td>
                            ...
                            <div class="tab-content">
                                @foreach (config('app.locales') as $key => $locale)
                                    <div class="tab-pane fade in @if ($key == 0) active @endif" id="tab_item_locale_new_{!! $locale !!}_{!! $item_key !!}">
                                        @include('views.teacher.tabs.item_locale', ['id' => $item_key, 'key' => 'new'])
                                    </div>
                                @endforeach
                            </div>
                        </td>
                    ...
                    @foreach (config('app.locales') as $key => $locale)
                        <div class="tab-pane fade in @if ($key == 0) active @endif" id="tab_item_locale_new_{!! $locale !!}_replaseme">
                            <div class="form-group required">
                                <input data-name="items[new][replaseme][{!! $locale !!}][name]" placeholder="@lang('labels.name')" class="form-control input-sm">
                            </div>

                            <div>
                                <textarea data-name="items[new][replaseme][{!! $locale !!}][content]" placeholder="@lang('labels.content')" rows="2" class="form-control input-sm content-textarea" id="items.new.replaseme.{!! $locale !!}.content" data-route="{!! route('admin.elfinder.ckeditor4') !!}"></textarea>
                            </div>
                        </div>
                        {{--@include('partials.tabs.ckeditor', ['id' => 'items.new.replaseme.'. $locale .'.content'])--}}
                    @endforeach
                </div>
            </td>
            <td>
